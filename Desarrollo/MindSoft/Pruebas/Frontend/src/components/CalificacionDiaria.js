...
export default function CalificacionDiaria({ visible, onClose }) {
  ...
  const handleEnviarEmociones = async () => {
    ...
    try {
      const moodRating = {
        date: getLocalDay(),
        mood_detail: selectedEmojis.map((emoji) => ({
          mood_id: emojisAnimo.find((item) => item.emoji === emoji).id,
        })),
      };
              ...
              {emojisCalif.map((item, index) => (
                <View
                  ...
                  style={[
                    ...
                    selectedFeeling === item.emoji &&
                      styles.selectedEmojiContainer1,
                  ]}
                >
                  <TouchableOpacity
                    key={index}
            ...
            <View style={styles.emojiEstadoAnimo}>
              {emojisAnimo.map((item, index) => (
                ...
                >
                  <TouchableOpacity
                    key={index}
                    onPress={() => handleSelectAnimo(item.emoji)}
                  >
