...
def get_daily_rating(db: Session = Depends(get_db), user: UserToJwt = Depends(get_current_user)):
...
def create_daily_rating(daily_rating: DailyRatingCreate, db: Session = Depends(get_db)):
