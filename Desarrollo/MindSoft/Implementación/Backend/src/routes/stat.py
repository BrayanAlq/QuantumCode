...
def get_stats_by_week(month_stats: MonthStats, db: Session = Depends(get_db), user: UserToJwt = Depends(get_current_user)):
    moods = get_moods_stats_by_week(db, user, month_stats)
    rating = get_rating_days_by_week(db, user, month_stats)
